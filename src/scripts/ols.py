...
for j in At.T:
    ...
    ols = np.linalg.inv(X.T @ X) @ X.T @ np.log(j)
    k_lin = np.append(k_lin, -ols[0])
    popt, pcov = curve_fit(first_order, t, j, p0=[k, A0])
...
titles.append("b")
axes[-1].plot(t / 100, At[:, 0], marker='.', color=fp.colors[0])
axes[-1].set_ylabel('$[\mathrm{H_2O_2}]_t$ / molm$^{-3}$')
axes[-1].set_xlabel('$t$ / s')
...
axes[-1].axvline((k_non / k).mean(), color=fp.colors[2], lw=1)
...
axes[-1].axvline((k_lin / k).mean(), color=fp.colors[0], lw=1)
...
f.close()
f = open(paths.output / 'non_mean_ols.txt', 'w')
f.write(r'\num{' + f'{np.mean(k_non / k):.2f}' + r'}')
f.close()

f = open(paths.output / 'lin_ci_ols.txt', 'w')
ci_lin = np.percentile(k_lin / k, [2.5, 97.5])
f.write(r'\numrange{' + f'{ci_lin[0]:.2f}' + r'}{' + f'{ci_lin[1]:.2f}' + r'}')
f.close()
f = open(paths.output / 'non_ci_ols.txt', 'w')
