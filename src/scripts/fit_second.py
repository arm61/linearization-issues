...
f.write(r'\SI{' + f'{k_lin.mean() - true_k:.3e}' + r'}{\per\mole\meter^3\per\second}')
f.close()
f = open(paths.output / 'non_bias.txt', 'w')
f.write(r'\SI{' + f'{k_non.mean() - true_k:.3e}' + r'}{\per\mole\meter^3\per\second}')
f.close()
f = open(paths.output / 'bias_ratio.txt', 'w')
f.write(r'\num{' + f'{(k_lin.mean() - true_k) / (k_non.mean() - true_k):.3f}' + r'}')
f.close()
