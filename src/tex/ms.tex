...
Consider the decomposition of hydrogen peroxide \ce{H2O2} in presence of excess cerium(III) ion (this example has been taken from Ref.~\cite{monk_math_2010}), which follows first-order rate kinetics.
...
\end{equation}
%
where, $[\ce{A}]_t$ is the concentration of the reactant \ce{A} (hydrogen peroxide) at time $t$, $[\ce{A}]_0$ is the initial concentration and $k$ is the rate constant. 
...
% It is standard to assume these random uncertainties follow a normal (or Gaussian) distribution \cite{monk_math_2010}. 
...
% Linearisation is commonly introduced to simplify the determination of reaction rate order, reaction rate temperature dependence, and many other applications \cite{perrin_linear_2017,hites_calculating_2017}.
% Where the dependent variable is 


% Can be useful tool to quickly identify the reaction order by plotting the data. ``Generally, we draw a series of graphs until the choice of axes yields a linear graph''.

% Problem is when the dependent variable is changed.
